...
class ModuleTopic_MapperTopic extends Mapper
{
	...
	public function UpdateTopicRead(ModuleTopic_EntityTopicRead $oTopicRead)
	{
		...
		$res = $this->oDb->query(
			$sql,
			$oTopicRead->getCommentCountLast(),
			$oTopicRead->getCommentIdLast(),
			$oTopicRead->getDateRead(),
			$oTopicRead->getTopicId(),
			$oTopicRead->getUserId()
		);
		return $this->IsSuccessful($res);
	}
